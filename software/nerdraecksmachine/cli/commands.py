...
from nerdraecksmachine.cli.configuration import built_in_profiles
from nerdraecksmachine.cli.supporting_functions import verify_configuration, set_fs_read_only, check_fs_read_only
from nerdraecksmachine.core.Core import StepperInterface
...
def configure(get_list, set_profile):
    ...
    if get_list:
        ...
        if set_profile not in built_in_profiles.keys():
            ...
            config_dir = user_config_dir('nerdraecksmachine')
            if not os.path.exists(config_dir):
                os.makedirs(config_dir)

            # We write our file
            with open(os.path.join(config_dir, 'current_nerdraecksmachine_profile'), 'w') as conf_file:
